...
parser.add_argument(
    "-m", "--matrix", type=str, dest="matrix", help="matrix in npz format", default=None
)
...
def run():
    ...
    inF = read_files(args.matrix, args.pbych5, xgiF, ygiF, basisF, coefF)
    V = inF["V"]
    xgi = inF["xgi"]
    ygi = inF["ygi"]
    W = inF["basis"]
...
def read_files(npz=None, h5=None, xgi=None, ygi=None, basis=None, coef=None):
    """
...
def stat_H(o_cell_class, o_cell_sparseness, o_entropy, ygi, prefix):
    ...
    head = "\t".join(
        ["ygi", "index", "class0", "class1", "contributes", "sparseness", "entropy"]
    )
...
def gerenate_files(o_stat_H, o_stat_W, rank, V, prefix):
    for r in range(rank):
        ...
        idx = np.concatenate(
            np.where(
                (o_stat_W[:, 2].astype(float).astype(int) == r)
...
def def_region_class(normW):
    ...
    return {
        "class0": class0,
        "contributes": contributes,
        "med_contribute": med_contribute,
        "selt_med_idx": selt_med_idx,
    }
...
def stat_W(o_region_class, o_featureScore_kim, xgi, prefix):
    ...
    statW = np.column_stack(
        (index, class0, contributes, fs, selt_fs_list, selt_med_list)
    )
    head = "\t".join(
        [
            "chr",
...
def cal_tSNE(X_dist, p):
    X_transformed = TSNE(
        n_components=2, perplexity=p, random_state=1, verbose=2, metric="precomputed"
    ).fit_transform(X_dist)
    return X_transformed
...
def plot_silhouette_tsne(o_silhouette, X_transformed, o_stat_H, rank, prefix):
    ...
    ax1.set_xlabel(
        " ".join(["The silhouette coefficient values (mean:", str(silhouette_avg), ")"])
    )
    ...
    plt.suptitle(
        (
            "Silhouette analysis for tSNE clustering on coefficient matrix H "
